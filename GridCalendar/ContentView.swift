...
//  GridCalendar
//
//  Created by fjunior on 18/10/22.
//
...
struct ContentView: View {
    ...
    var body: some View {
        ScrollView {
            LazyVGrid(columns: layout, pinnedViews: [.sectionFooters]){
                ForEach(year, id: \.name) {
                    month in Section(header: Text(verbatim:
                                                    month.name).font(.headline)) {
                        ForEach(month.days) { day in
                            Capsule()
                                .overlay(Text("\(day.value)").foregroundColor(.white)).foregroundColor(.blue)
                                    .frame(height: 40)
